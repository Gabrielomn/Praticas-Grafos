...
public class ImportSimpleGraphGML
{
    ...
    private static Graph<DefaultVertex, RelationshipEdge> graphgml;

    public Graph ImportSimpleGraphGML(String path)
    {
        ...
        EdgeProvider<DefaultVertex, RelationshipEdge> ep1 = (from, to, lable, attributes) -> new RelationshipEdge(from, to, attributes);
        ...
        // importado.
        graphgml = new SimpleGraph<>(RelationshipEdge.class);

        // Bloco responsavel pela importacao do grafo.
        try
        { // tenta importar o grafo...
            gmlImporter.importGraph(graphgml, ImportGraph.readFile(path)); // chama o metodo de importacao...
        } catch (ImportException e)
        { // caso seja lancada alguma excessao...
            throw new RuntimeException(e); // ela eh retornada para o usuario.
        }

        return this.graphgml;
    }
